...
class BaseBCMILoader(ABC, Generic[E]):
  ...
  @abstractmethod
  def music_iterator(self) -> Iterator[Tuple[MusicFilename, MusicData]]:
    """
    Iterate over all music data in the dataset.

    Yields:
        Tuple of (MusicFilename, MusicData) pairs containing music reference and data
        (could be WavRAW, MusingMusicIdData, or other MusicData subclasses)
    """
    pass
...
class BCMIMusingLoader(BaseBCMILoader[RawEeg]):
  """
  Loader for MUSIN-G dataset.

  The MUSIN-G dataset contains EEG recordings from participants listening to
  12 different songs spanning various genres. Each subject listened to all 12 songs,
  with each song presented in a separate session. The dataset focuses on neural
  responses to diverse musical stimuli.

  Experimental Design:
      - Purpose: Study neural responses to music listening across genres
      - Participants: 20 healthy adults
      - Sessions: 12 per subject (one per song)
      - Task: Passive music listening
      - Songs: 12 tracks covering diverse genres (Electronic, Rock, Classical, etc.)
      - Duration: ~2 minutes per song
      - Sampling Rate: 250 Hz
      - Format: BIDS-compliant EEGLAB .set files

  Song Information:
      The 12 songs span various genres including:
      - Electronic/Dance (Deep House, Electronic Dance, Ambient)
      - Rock/Alternative (Indie, Goth Rock, Progressive Rock)
      - Indian Classical & Semi-Classical
      - Jazz
      - New Age

  Data Structure:
      Each session corresponds to one complete song listening experience.
      Sessions are numbered 01-12, matching the song IDs (1-12).

  Usage:
      >>> loader = BCMIMusingLoader('/path/to/musin_g_data')
      >>> data = loader.load_all_subjects()
      >>>
      >>> # Access subject data
      >>> subject_data = loader.load_subject_data('001')
      >>> song1_data = subject_data['1']['1']  # session 1, run 1
      >>> raw_eeg = song1_data['raw']
      >>>
      >>> # Iterate over trials
      >>> for trial in loader.trial_iterator():
      ...     print(f"Subject {trial.subject}, Song {trial.session}")
      ...     print(f"EEG duration: {trial.eeg_data.length_seconds():.1f}s")

  Notes:
      - The dataset uses sessions to represent different songs (ses-01 to ses-12)
      - Each session has one run containing the full song listening experience
      - Behavioral ratings (enjoyment, familiarity) are available in stimuli/Behavioural_data
      - No audio files are included in the dataset (only EEG and metadata)
  """

  def __init__(self, root_path: str, dataset_name: str = "musin-g"):
    super().__init__(root_path, dataset_name)

  def _get_available_sessions(self, subject_id: str) -> List[str]:
    """Get available sessions for a subject (returns sorted list of session IDs).

    For MUSIN-G, sessions correspond to the 12 songs (ses-01 to ses-12).
    """
    try:
      sessions = get_entity_vals(str(self.root_path), "session")
      # Ensure sessions are sorted numerically
      return sorted(sessions)
    except Exception:
      # Fallback: check for session directories
      subject_path = self.root_path / f"sub-{subject_id}"
  ...
  def _get_available_runs(self, subject_id: str, session: str = "") -> List[str]:
    ...
    For MUSIN-G, each session has exactly one run with the same number as the session.
    """
    if session:
      # Each session (ses-01, ses-02, etc.) has one run (run-1, run-2, etc.)
  ...
  def load_subject_data(
    ...
    """Load data for one MUSIN-G subject across all sessions.

    Overrides base class to handle MUSIN-G specific task naming.
    """
    self.data[subject_id] = {}
    sessions = self._get_available_sessions(subject_id)
    ...
    for session in sessions:
      ...
      for run in runs:
        try:
          # MUSIN-G specific: use task='MusicListening' and run number
          bids_path = BIDSPath(
            subject=subject_id,
            session=session,
            task="MusicListening",
          ...
          specific_events = self._process_events_specific(events_df, run_data)
          run_data["processed_events"].update(specific_events)
          run_data["n_trials"] = 1  # Each session is one complete song trial

          self.data[subject_id][session_key][run] = run_data

          print(
            f"  ✓ Song {session} (run {run}): {raw.times[-1]:.1f}s, "
            f"{len(events_df)} events"
          )

        except Exception as e:
          print(f"  ✗ Song {session} (run {run}): {str(e)[:80]}...")

    return self.data[subject_id]

  def trial_iterator(self) -> Iterator[TrialRow[RawEeg]]:
    """
    Iterate over EEG trial snippets for MUSIN-G data.

    For MUSIN-G dataset, each session (song) is treated as one complete trial.
    The entire recording for each song is returned as a single trial.

    Yields:
        TrialRow: Trial data with song ID, raw EEG, and metadata
    """
    for subject, session, run, r in self.loader_data_iter():
      # Each session corresponds to one song (1-12)
      song_id = int(session)
      music_filename = MusicFilename.from_musicid(MusingMusicId(song_id=song_id))

      # Use the entire recording as one trial
      # Use session-specific trial_id to ensure uniqueness
      yield TrialRow(
        dataset="musin-g",
        subject=subject,
        session=session,
        run=run,
        trial_id=f"song_{session}",
        eeg_data=RawEeg(raw_eeg=r["raw"].copy()),
        music_filename=music_filename,
      )
  ...
  def _get_experimental_info(self) -> Dict[str, Any]:
    return {
      "paradigm_type": "Music Listening",
      "trial_structure": "12 complete songs, one per session",
      "music_type": "Commercial music tracks (12 diverse genres)",
      "sessions_per_subject": 12,
      "songs": 12,
      "behavioral_data": "Enjoyment and familiarity ratings",
      "description": "Neural responses to passive listening of 12 diverse musical pieces",
    }
  ...
  def _process_events_specific(
    ...
    """
    Process MUSIN-G specific events.

    MUSIN-G uses markers like 'bgin' (begin), 'stim', 'clyp', 'opyp' for timing.
    Since each session is one complete song trial, we return basic event information.
    """
    try:
      # Extract timing markers if present
      timing_markers = []
      ...
      return {
        ...
        "num_markers": len(timing_markers),
      }
    except Exception as e:
      print(f"    Warning: Could not process MUSIN-G events - {str(e)[:50]}")
      return {}
...
def create_bcmi_loader(dataset_path: str) -> BaseBCMILoader:
  ...
  Supported Dataset Types:
      ...
      - musin-g: Music listening with 12 diverse songs
  """
  ...
  loader_map = {
    ...
    "musin-g": BCMIMusingLoader,
    "musin_g_data": BCMIMusingLoader,  # Alternative naming
  }
